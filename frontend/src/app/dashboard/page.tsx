...
const parseRgbStringToHex = (rgbString: string): string => {
  ...
  if (!match || match.length < 3) return "#3B82F6"; // Default blue
  const [r, g, b] = match.slice(0, 3).map(Number);
  return rgbToHex(r, g, b);
};

// Parse RGB string format to color name
const getRgbColorName = (rgbString?: string | null): string => {
  if (!rgbString) return "Blue"; // Default
  const match = rgbString.match(/\d+/g);
  if (!match || match.length < 3) return "Blue";
  const [r, g, b] = match.slice(0, 3).map(Number);
  return rgbToColorName(r, g, b);
};

// Convert color value (hex or rgb string) to hex format
const getColorHex = (colorValue?: string | null): string => {
  ...
  if (colorValue.includes("rgb") || colorValue.includes(",")) {
    return parseRgbStringToHex(colorValue);
  }
            ...
            <div className="relative rounded-[15px] bg-[rgba(9,9,11,0.9)] px-6 py-6">
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    setProfileSaving(true);
                    // Convert hex color to RGB format for database storage
                    const rgbColor = hexToRgb(highlightColorInput);
                    const rgbString = `${rgbColor.r}, ${rgbColor.g}, ${rgbColor.b}`;

                    await updateHighlightColor(rgbString);
                    showToast({
                      message: `Highlight color saved: ${highlightColorInput.toUpperCase()}`,
                      title: "Saved",
                      variant: "success",
                    });
